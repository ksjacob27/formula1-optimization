...
\noindent Configuration: hidden size 64, 2 layers, dropout 0.2, batch-first. \\
Total parameters: approximately 50,000. \\
Training uses Adam optimizer~\cite{kingma2015} with learning rate $10^{-3}$, ReduceLROnPlateau scheduler (patience=3, factor=0.5), gradient clipping at norm 1.0, and early stopping at patience=7 on validation loss.

An LSTM with additive attention~\cite{bahdanau2015} was also evaluated. Attention computes a learned weighted sum over all 10 hidden states before 
the output projection. This variant was the worst performer (MAE $-22.5\%$ vs.\ baseline), as discussed in Section 4.3.
...
\textbf{Worn tire projection} (\texttt{predict\_worn}): Starting from the last 10 observed laps, the model auto-regressively predicts 20 future laps.
 StintLength increments by 1 at each step while FuelLoad decrements proportionally, and compound one-hot remains constant.

\textbf{Fresh tire projection} (\texttt{predict\_fresh}): Seeds from the earliest low-tyre-life laps of the target compound
 (StintLength $< 3$ normalized laps) from earlier in the race, resetting StintLength to 0.
...
\end{equation}
where $L_\text{pit}$ is the normalized pit stop time loss (22 seconds / max lap time). Pit loss values are sourced per circuit than remaining constant. Las Vegas, Singapore, Monaco, and Miami are shown to be deviating notably from the 22 second default value.
