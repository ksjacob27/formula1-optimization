...
Formula 1 regulations require each driver to use at least two different tire compounds during a dry race. The three primary 
...
significant degradation; HARD tires are considered the slowest but last $\sim$35--50 laps and generate less grip throughout. Furthermore, tire degradation is influenced by numerous factors such as track temperature, compound type, driving attitude, etc.
...
architectures for sequential prediction tasks. Both maintain a hidden state that ummarizes prior context, updated at each
 timestep through gating mechanisms.

LSTM cells use three gates (input, forget, output) and a separate cell state, giving them strong capacity for long-range
...
As initially stated in my proposal, Commercial F1 strategy tools (e.g., McLaren Electronic Systems, Williams Racing Analytics) use physics-based tire models combined with Monte Carlo simulations over opponent strategies \cite{mcaleer2023}. These systems require real-time telemetry and proprietary tire compound data, neither of which are available for public use.

Academic work on F1 strategy is severely limited due to the lack of publicly available data. Heilmeier \ \cite{heilmeier2020} develop a minimum-lap-time race simulation that incorporates tire degradation, fuel load, and safety car probability, using an energy-based tire model fitted on historical sector time data. Their approach requires compound-specific parameters obtained from team data and focuses on pre-race strategy planning rather than real-time window detection.

Tulabandhula and Rudin \cite{tulabandhula2014} apply machine learning to sports decision problems more broadly, demonstrating that data-driven approaches can approximate expert judgment in complex sequential settings. More recently, reinforcement learning has been applied to F1 pit stop timing \cite{cheng2021}, treating the strategy decision as a Markov decision process. These approaches are severely limited by lack of resoucres as they require simulator environments that are difficult to calibrate without access to official team data.

\subsubsection{Tire Degradation Modeling}

Tire degradation has been modeled as a polynomial function of stint length in physics-based simulators \cite{heilmeier2020}. Data-driven approaches 
include Gaussian process regression over lap time residuals \cite{loiacono2011} and recurrent models trained on historical sector times. To my
 knowledge, no published work applies compound-specific GRU models to pit stop window detection using publicly available lap data.
